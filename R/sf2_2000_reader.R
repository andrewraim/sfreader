#' @export
sf2_2000_reader = R6Class("sf2_2000_reader",
	private = list(
    	path_to_files = NULL,
		geo_list = NULL,
		readGeoFile = function() {
			...
			ff = list.files(path = private$path_to_files, pattern = "geo.uf2")
			if (length(ff) != 1) {
			...
			geo_dat = read_fwf(file = geo_file,
				col_positions = fwf_widths(sf2_2000_geo_cols[['FIELD SIZE']]))

			# Assign column headers based on access specifications
			colnames(geo_dat) = sf2_2000_geo_cols[['FIELD']]
	...
	public = list(
		...
		write_sqlite = function(conn, target_tables = NULL)
		{
			...
			dbWriteTable(conn, "TableNames", sf2_2000_tables)

			if (is.null(target_tables)) {
				logger("Saving all available tables to the database\n")
				target_tables = sf2_2000_tables$NUMBER
			}
			...
			for (idx_segment in 1:nrow(segments)) {
				...
				data_files = list.files(
					...
					pattern = sprintf(".*%02d.uf2", target_segment),
					full.names = TRUE, recursive = TRUE)

				dat_segment = sf2_2000_tables %>%
					filter(as.integer(SEGMENT) == target_segment) %>%
					filter(NUMBER %in% target_tables)

				segment_dd = sf2_2000_geoheader_dd %>%
					filter(as.integer(SEGMENT) == target_segment) %>%
					arrange(SORT_ID)
				...
				for (idx_file in 1:length(data_files)) {
					...
					for (idx_tables in 1:nrow(dat_segment)) {
						...
						idx_col = which(segment_dd$TABLE == table_name | is.na(segment_dd$TABLE))

						dat_selected = dat[,idx_col] %>%
